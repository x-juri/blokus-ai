...
import { PieceInventoryPanel } from "./PieceInventoryPanel";
import { SuggestionsPanel } from "./SuggestionsPanel";

const defaultWorkbenchAgent: AgentConfig = {
...
export function WorkbenchMode() {
  ...
  async function handleSuggest() {
    ...
    try {
      const ranked = await suggestMoves(
        boardState,
        topK,
        simulations,
        candidateLimit,
        defaultWorkbenchAgent
      );
    ...
    try {
      ...
      setStatusMessage(
        suggestion.move.is_pass
          ? `Passed turn for ${suggestion.move.color}.`
          : `Applied ${suggestion.move.piece_id} for ${suggestion.move.color}.`
      );
    } catch (error) {
